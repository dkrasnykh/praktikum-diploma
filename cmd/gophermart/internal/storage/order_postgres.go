...
func (o *OrderPostgres) Add(ctx context.Context, userID int, orderNumber string) error {
	_, err := o.db.Query(ctx, "INSERT INTO rewards (user_id, order_number, status) values ($1, $2, $3)",
		userID, orderNumber, models.New)
	return err
}

func (o *OrderPostgres) GetAll(ctx context.Context, userID int) ([]models.Order, error) {
	rows, err := o.db.Query(ctx, "SELECT (order_number, status, accrual, updated_at) "+
...
func (o *OrderPostgres) Update(ctx context.Context, order models.AccrualResponse) error {
	if order.Accrual != nil {
		...
		_, err := o.db.Exec(ctx, "UPDATE rewards SET status=$1, accrual=$2, updated_at=$3 WHERE order_number=$4",
			order.Status, accrual, time.Now(), order.Order)
		return err
	}
	_, err := o.db.Exec(ctx, "UPDATE rewards SET status=$1, updated_at=$2 WHERE order_number=$3",
...
func (o *OrderPostgres) GetUserIDByNumber(ctx context.Context, orderNumber string) (*int, error) {
	var id int
	var err error
	row := o.db.QueryRow(ctx, "SELECT user_id FROM rewards WHERE order_number=$1 LIMIT 1", orderNumber)
